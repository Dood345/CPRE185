...
int main()
{
	...
	do
	{
		...
		//buttonToggle(b_X, &timeX); //can't realize how to do this with a function
		
		if (b_X == 1) {							//if button is pressed
			if (checkX != b_X) {				//if last recorded state is different from button being pressed
				toggleX = (toggleX + 1)%2;		//change the toggle flag to on
				checkX = 1;						//change recorded state to pressed
			}
			else if (checkX == b_X) {			//if last recorded state is the same as current one
				timeX = timeX + b_X;			//add time to the time pressed counter
			}
		}
		else if (b_X == 0) {					//if button is not pressed
			if (checkX != b_X) {				//if button was previously pressed
				checkX = 0;						//change recorded state to not pressed
			}
			/*else if (checkX == b_X) {			//if button was previously not pressed
				checkX = 0;						//who cares?
			}*/
		}
		
		//timeX = timeX + b_X;			prototype for toggle
		//toggleX = (timeX / 5)%2;		prototype for toggle
		
		//printf("%d\n", toggleX);		toggle testing
		
		if (toggleX) {//MODIFY to use a toggle
			// Scale your output value √
			scaled_value = scaleRadsForScreen(pitch_rad);
		}
		else if (!(toggleX)) {//MODIFY to use a toggle
			// Scale your output value √
